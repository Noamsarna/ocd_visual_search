...
var randomY = [];
var randomNumber = -1;
jsPsych.plugins["p5vs_yn_small_grid"] = (function () {

  var plugin = {};

  plugin.info = {
    name: 'p5vs_yn_small_grid',
    parameters: {
      title: {
    ...
    let sketch = function (p) {
      ...
      function presentArray(triall, randomNumber, randomNumbers, randomX, randomY) {
        ...
        for (i = 1; i < trial.set_size; i++) {
          ...
          if (trial.isCDistractor || trial.is_practice_for_c_trials) {
            trial.distractor_function(p, p.millis(), du, randomNumbers[i], positions[i][2], positions[i][3])
          } else {
            trial.distractor_function(p, p.millis(), du, 0, positions[i][2], positions[i][3])
          }
          p.pop()
        }
      }
