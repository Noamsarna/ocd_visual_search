...
hist(OCI_df$oci_overall, breaks = 'FD', xlab = "OCI-R total score", main = "OCI-R histogram", cex.main = 1.25)
#dev.off()
...
hist(DASS_df$DASS_anxiety, breaks = 20, xlab = "DASS anxiety total score", main = "DASS anxiety histogram", cex.main = 1.25, xlim = c(0, 20))
#dev.off()

#depression
#png("../ocd_visual_search/docs/figures/DASS_dep_hist.png", width = 1200, height = 800, res = 300)
hist(DASS_df$DASS_depression, breaks = 20, xlab = "DASS depression total score", main = "DASS depression histogram", cex.main = 1.25, xlim = c(0, 20))
#dev.off()
...
exp1_acc_df %>% filter(OCI_quantile==1 | OCI_quantile==4) %>%
  group_by(OCI_quantile, target_present) %>% 
  summarise(mean_acc = mean(accuracy))

#t.test comapre groups acc 
...
# Exclusion Criteria  
  ...
  Participants were excluded if they made more than 15% errors in the main part of the experiment or for having extremely fast  (below 100 milliseconds) in more than 25% of the trials. Participants were also excluded from the analysis if they failed one or more of the attention checks. In total, 109 out of 1007 participants were excluded from the analysis. For the remaining participants, error trials and trials with response times below 100 milliseconds were excluded from the response-time analysis.
...
#regualr t-test for sanity check 
t.test(diff_mean_RT~OCI_quantile, data=H3.df_mean_diff, alternative="two.sided")
#Bayesien t-test 
