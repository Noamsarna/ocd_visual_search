...
Theories on obsessive-compulsive disorder (OCD) emphasize the pivotal role of pathological doubt in the disorder's phenomenology [@darElucidatingMechanismUncertainty2004; @darSeekingProxiesInternal2021; @rasmussenClinicalFeaturesPhenomenology1989; @reedObsessionalExperienceCompulsive1985a; @shapiroNeuroticStyles1965]. This persistent doubt is reflected in lowered confidence in memory, decision making, perception and other cognitive functions, which give rise to repetitive checking rituals that, paradoxically, only serve to intensify the doubt [@vandenhoutRepeatedCheckingCauses2003a]. In the lab, doubt and checking behavior are commonly manifested in slow reaction times. For instance, in a study by @sarigObsessivecompulsiveTendenciesAre2012Sarig, participants with high and low obsessive-compulsive tendencies (OC+; OC- respectively) were asked to judge the exact midpoint on a continuum of colors. In another study by @rotgeChallengingTaskAssessment2008, patients with OCD had to compare two images and determine whether they were identical or different. In both studies, obsessive-compulsive tendencies were associated with slower reaction times, presumably driven by the need to check. Such findings have been interpreted as indicating that OC+ participants require more evidence when faced with uncertainty in perceptual decision-making tasks [@bancaEvidenceAccumulationObsessiveCompulsive2015; @hauserMetacognitiveImpairmentsExtend2017d].
...
```{r Exp2_load_data, echo=FALSE, warning=FALSE, cache=TRUE}
...
# Results
```{r Exp2_analyze_data, echo=FALSE, warning=FALSE, cache=TRUE}
