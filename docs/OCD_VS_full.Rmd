...
*Hypotheses 1 and 2 - Task validation * - To validate our paradigm structure and to assess whether we successfully created an easier search by leveraging search asymmetries (i.e., switching between the target and distractors), we first examined the difference in slopes between the two search types (easy/hard), regardless of target presence (pre-registered hypothesis H1). As anticipated, a one-tailed paired t-test demonstrated a steeper slope for the difficult search `r printnum (H1.desc[2,3])` compared to the easy search  `r printnum (H1.desc[1,3])`, `r apa_print(t.test(estimate~search_type, data=search_slopes_df, paired=T))`. Furthermore, a one-tailed paired t-test revealed that target-present slopes in the hard search `r printnum (H2.desc[2,3])` were steeper than target-absent slopes in the easy search `r printnum (H2.desc[1,3])`, `r apa_print(t.test(estimate~group, data=H2.df, paired=T))` (pre-registered hypothesis H2).
