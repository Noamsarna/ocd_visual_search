...
#library(groundhog)
#pkgs <- c('tidyverse', 'broom', 'nlme', 'lme4', 'lmerTest', 'ggpubr', 'cowplot', 'caret', 'papaja')
#groundhog.library(pkgs,'2022-04-22')
...
In contrast to our hypothesis, we didn't find an interaction between group and target presence in the direction we expected `r apa_print(afex::aov_ez("subj_id", "mean_RT", H3.df, between = "OCI_quantile", within = "target_present"))`. There is a marginaly significant interaction to the opposite direction, in which OC- are having longer search time in target absent trials, but not in target present trials. 
...
Hypothesis 5 isn't met. There isn't a significant interaction between group and target present in the easy search `r apa_print(search_slopes_df %>% filter(OCI_quantile==1| OCI_quantile==4) %>% filter(search_type=='CinO') %>%aov(estimate ~OCI_quantile*target_present, data=.))` 
...
Consistent with our hypotesis, we found a significance difference between the slope of the easy search (C in O) and the slope of the hard search (C in O) in the first trials of each block type `r apa_print(t.test(estimate~search_type, data=search_slopes_df_with_first_t, paired=T))`. This significant difference between the search slopes indicate that subjects had some knowledge of the search assymetry already in the first trials of the task. 
...
```
We didn't find an interaction between search type and group on first trials slopes `r apa_print(search_slopes_df_first_t %>%aov(estimate~OCI_quantile * search_type, data = .) %>% summary(.))`
