...
#include only subjects who have a valid 4 first trials
...
Consistent with our hypotesis, we found a significance difference between the slope of the easy search (C in O) and the slope of the hard search (C in O) in the first trials of each block type. This significant difference between the search slopes indicate that subjects had some knowledge of the search assymetry already in the first trials of the task. 
...
```
The interaction between OC and target presence remain not significant when analyzing the results using the entire quartile of OC.  
