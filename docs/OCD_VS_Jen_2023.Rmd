...
#need to add the figures. knitr::include_graphics("../docs/figures/direct_replication_plot.png")
...
In line with our hypothesis, search slopes for the easy absent search were shallower,than search slopes in the hard present condition (`r apa_print(t.test(estimate~group, data=H2.df, paired=T))$full_result`).  
...
```{r direct-replication, echo=FALSE, cache=TRUE, caption='Direct replication'}
...
In contrast to our hypothesis, we didn't find an interaction between group and target present in the direction we expected. There is a marginaly significant interaction (`r apa_print(aov(RT~OCI_quantile *target_present, data=toffolo_data %>%filter(OCI_quantile==1| OCI_quantile==4) ))$full_result`) to the opposite direction, in which OC- are having longer search time in target absent trials, but not in target present trials. 
...
(`r apa_print(aov(estimate ~ OCI_quantile*target_present, data=search_slopes_df %>%filter(OCI_quantile==1| OCI_quantile==4) %>% filter(search_type=='CinO')))$full_result`).
...
Model comparison shows that both models have the same R-sqaured value [MM: was this statistically quantified? NS:No. I'm not sure what statistical test should be done here, though the R-sqaured are almost identical], therefore we can reject the hypothesis that target-presence affects OC+ search time to a greater extent than search type on OC+ [MM: can we reject an hypothesis because we don't find a significant difference?]. 
...
[MM: how do we know the t test function knows to match the two slopes by subj_id? I don't see a grouping variable in the formula. NS: Since every participant has two search types options no? this is like before/after experiment. Were would we provide a grouping variable?]

We found a significance difference between the slope of the easy search (C in O) and the slope of the hard search (C in O) in the first trials of each block type (`r apa_print(t.test(estimate~search_type, data=search_slopes_df_first_t, paired=T))$full_result`). This significant difference between the search slopes indicate that subjects had some valid expectations about search difficulty, allowing them to terminate the easy search faster than the hard search.  
