...
The experiment described in this study was adapted from @mazor2022efficient, with stimuli created to replicate the ones used in the @toffolo2013mild. All elements (distractors and target) and their placement on the search grid were made to replicate as closely as possible the paradigm used in @toffolo2013mild. 
...
In line with our hypothesis and paradign structure, search slopes between the two search type, followed the expected pattern with a steeper slope for the hard search (O in C; `r apa_print(t.test(estimate~search_type, data=search_slopes_df, paired=T))$full_result`).
...
```
  ...
  summary(.)
  [MM: things that don't appear within back ticks (either as part of a chunk or with the backtick-r-backtick notation) are not evaluated as R code.]

In contrast to our hypothesis, we didn't find an interaction between group and target present in the direction we expected. There is a marginaly significant interaction ([MM: why is this not generated by R?] p=0.7) to the opposite direction, in which OC- are having longer search time in target absent trials, but not in target present trials. 
...
[MM: this should be in a chunk, and stats should appear in the next paragraph.]
  search_slopes_df %>% 
  filter(OCI_quantile==1| OCI_quantile==4) %>% 
...
#NS: I need to change this model into a MLM since each participant have more than one observation. 
# Do you know how to implement it with caret? 
...
search_slopes_df%>% 
    ...
    train(estimate ~ 1+OCI_group+ search_type * target_present +search_type *OCI_group,
               data =. , method = "lm", trControl = ctrl)
...
m_2 <- 
  ...
  train(estimate ~ 1+OCI_group+ search_type * target_present +target_present *OCI_group,
               data = ., method = "lm", trControl = ctrl)
...
Model comparison shows that both models have the same R-sqaured value [MM: was this statistically quantified?], therefore we can reject the hypothesis that target-presence affects OC+ search time to a greater extent than search type on OC+ [MM: can we reject an hypothesis because we don't find a significant difference?]. 
...
#MM: I think so, I rejected them here. notice I also changed the summary statistics to median
...
[MM: how do we know the t test function knows to match the two slopes by subj_id? I don't see a grouping variable in the formula]

Consistent with our hypothesis [MM: here and in other places you say "consistent with our hypothesis", but I'm not sure we actually pre-registered a directional hypothesis, just a null hypothesis that we are planning to test], we found a significance difference between the slope of the easy search (C in O) and the slope of the hard search (C in O) in the first trials of each block type (`r apa_print(t.test(estimate~search_type, data=search_slopes_df_first_t, paired=T))$full_result`). This significant difference between the search slopes indicate that subjects had some sort of metacognitive knowledge (valid expectations about search difficulty), allowing them to terminate the easy search faster than the hard search.  
...
#figure
  ...
  first_trials_df %>% 
  filter(OCI_quantile==1|OCI_quantile==4) %>% 
  mutate(OCI_quantile=factor(OCI_quantile, levels=c(1,4), labels=c('low','high'))) %>%
  ...
  search_slopes_df_first_t %>% 
  aov(estimate~OCI_quantile * search_type, data = .) %>% 
  summary(.)

```
We didn't find an interaction between search type and group on first trials slopes. 
...
``` {r full_range}
m_full_range <-lmer(estimate ~ 1+OCI_quantile+ search_type * target_present +target_present *OCI_quantile+
               (1|subj_id),
               data = search_slopes_df)
summary(m_full_range)

```
The interaction between OC and target presence remain not significant when analyzing the results using all four quartiles.  
...
m_anx_depp <-lmer(estimate ~ 1+OCI_quantile+ search_type * target_present +target_present *OCI_quantile+
