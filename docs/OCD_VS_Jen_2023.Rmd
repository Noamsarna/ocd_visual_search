...
prow<-
  plot_grid(
    ...
    one_fig_HOC  +theme(legend.position="none",

    ), 
    one_fig_LOC  +theme(legend.position = 'none',
...
#two plots with one legend 
plot_grid(prow+theme(text = element_text(size = 20)),
          legend,labels = "auto",
          label_size = 0,
          rel_widths = c(4, .4))
...
#plotting a figure of mean RT in the same strcuture as the percieved difficulty for comparison. 
RT_over_participant_oci %>% 
  filter(OCI_quantile==1 |OCI_quantile ==4) %>% 
...
#create a df for correlating percieved difficulty with actual search time 
...
#plot correlation between difficulty and search time 
search_correlation %>% 
  filter(OCI_quantile==1 | OCI_quantile==4) %>% 
  ggplot(aes(x = as.double(response), y = median_RT,
             color  = as.factor(OCI_quantile))) +
  geom_point() + geom_smooth(method = "lm", se=F) +
  ggpubr::stat_cor() +
  facet_wrap(~difficulty_rating) +
  theme_bw()+
  xlab('perceived difficulty')
